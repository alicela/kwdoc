...
| **US-0.9** | **Créer l'endpoint Genesis de lecture par `partitionId`** (côté Genesis) : renvoie les interrogations d'une partition en **cumulant les modes**, sous forme de **documents contenant chacun une liste d'interrogations×mode** ; **toutes les variables d'une interro×mode sont dans le même document** (jamais scindée entre deux documents) ; paginé (par documents). Champs par interro×mode : `interrogationId`, `usualSurveyUnitId`, `collectionInstrument`, `mode`, `questionnaireState`, `validationDate`, `isCapturedIndirectly`, variables collectées/externes/calculées (`varId`/`value`/`scope`/`iteration`), + `shortLabel` de partition. Sécurisé OIDC ; **OpenAPI publié**. *Chaque interro×mode complète dans un document → pivot par interro×mode, pipeliné (spec technique §4.1).* | **Prérequis dur** de US-0.4 / US-1.1 — l'appel par `partitionId` n'existe pas aujourd'hui (Genesis expose `campaignId`/`questionnaireModelId`). **Équipe Genesis.** | L |
...
|---|---|---|---|---|
| **US-1.0** 🔬 | **SPIKE — pivot « lignes de variables → tables larges par niveau »** : valider l'**agrégation conditionnelle pilotée par les métadonnées BPM** (colonnes = variables du niveau, typage, occurrence par `dense_rank`) sur un jeu réel RACINE + 1 boucle. **Solution proposée : spec technique §4.1.** Valider aussi : **pivot par interro×mode en flux** (variante B — chaque interro×mode complète dans un document) vs **staging + GROUP BY** (variante A) ; **parallélisme** (fan-out par interro×mode, DuckDB multi-cœurs, writer unique) — cf. schéma `../schemas/cible-pipeline-parallelisme.png`. Livrable : SQL-builder + POC **mesuré (RAM heap / temps / cœurs)** + choix variante A/B + décision `dense_rank` vs `iteration` (CL-STR-04) | RG-STR-01/03/04/06, CL-STR-03/04, NF-001/002 | — | M |
